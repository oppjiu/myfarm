...
<script>
    ...
    var seedStoreGridView = $.extend({}, $.fn.datagrid.defaults.view, {
        renderRow: function (target, fields, frozen, rowIndex, rowData) {
            ...
            let divStr = '<div class="mainBox_' + rowData['cropId'] + '" style="padding: 10px;border: 0;display: inline-block;">' +
                '<div style="display: table;width: 100px;border: red 1px solid;border-collapse:collapse;">' +
                '<div style="display: table-row;"><div style="display:table-cell;border: red 1px solid;"><p style="padding:10px;height: 100px;overflow-y:auto;">"' + rowData['tips'] + '</p></div></div>' +
                '<div style="display: table-row;"><div style="display:table-cell;border: red 1px solid;"><img style="display:block;width: 150px;" src="' + imgUrl + '" alt="种子图片"></div></div>' +
                '<div style="display: table-row;"><div style="display:table-cell;border: red 1px solid;padding: 10px;text-align: center;"><input type="button" value="我要购买"></div></div>' +
                '</div></div>'
            return $(divStr).prop('outerHT7ML');
        }
    });

    $(function () {
        $('#seedStoreWindow').window({
            title: '种子商店',
            width: '80%',
            height: '80%',
        });
        ...
        seedStoreGrid = $('#seedStoreGrid').datagrid({
            ...
            onLoadSuccess: function (obj) {
                ...
                for (let i = 0; i < data.length; i++) {
                    let content = $('<div style="color: black;font-weight: bold;">').append('名称：' + data[i]['cropName']).append('<br>')
                    ...
                    $('#mainBox_' + data[i]['cropId']).tooltip({
                        title: 'title',
                        trackMouse: true,
                        position: 'right',
                        content: content,
                ...
                console.log("$('#seedStoreWindow').height()", $('#seedStoreWindow').height());
                $("#seedStoreGrid").datagrid("resize", {
                    height: ($('#seedStoreWindow').height())
                });
            },
            columns: [[
                {
